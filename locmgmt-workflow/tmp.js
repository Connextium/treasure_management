...
var LC_ISSUED_EVENT_HASH = keccak256(toHex("LCIssued(uint256,address,address,uint256,address)"));
var LC_ACTIVATED_EVENT_HASH = keccak256(toHex("LCActivated(uint256)"));
var LC_SETTLED_EVENT_HASH = keccak256(toHex("LCSettled(uint256)"));
var LC_EXPIRED_EVENT_HASH = keccak256(toHex("LCExpired(uint256)"));
var LC_FUNDS_MINTED_EVENT_HASH = keccak256(toHex("LCFundsMinted(uint256,address,uint256)"));
var LOC_ACTIVATED_EVENT_HASH = keccak256(toHex("LocActivated(uint256,uint256)"));
var LOC_SETTLED_EVENT_HASH = keccak256(toHex("LocSettled(uint256,address,uint256,uint256)"));
var LOC_EXPIRED_EVENT_HASH = keccak256(toHex("LocExpired(uint256,uint256)"));
var FUNDS_RELEASED_EVENT_HASH = keccak256(toHex("FundsReleased(address,uint256,uint256)"));
function lcIssuedListener(runtime2, payload) {
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("LocManagement: LCIssued Event Detected");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  try {
    ...
    runtime2.log(`[Step 1] Decoded LCIssued event log: ${JSON.stringify(decodedLog, (_, v) => typeof v === "bigint" ? v.toString() : v)}`);
    const { args } = decodedLog;
    const { locNo, buyerAcc, sellerAcc, amount, locContractAddress } = args;
    runtime2.log(`[Step 1] LCIssued event detected`);
    runtime2.log(`[Step 1] LC Number: ${locNo}`);
    runtime2.log(`[Step 1] Buyer Account: ${buyerAcc}`);
    runtime2.log(`[Step 1] Seller Account: ${sellerAcc}`);
    runtime2.log(`[Step 1] Amount: ${amount.toString()}`);
    runtime2.log(`[Step 1] Loc Contract Address: ${locContractAddress}`);
    const txHash = bytesToHex(payload.txHash);
    const blockNumber = payload.blockNumber;
    runtime2.log(`[Step 2] Transaction Hash: ${txHash}`);
    runtime2.log(`[Step 2] Block Number: ${blockNumber}`);
    const lcData = {
      locNo,
      buyerAcc,
      sellerAcc,
    ...
    runtime2.log(`[Step 3] LC data prepared for processing`);
    runtime2.log(`[Step 4] Business Logic Integration:`);
    runtime2.log(`[Step 4] - Store Loc contract address: ${locContractAddress}`);
    runtime2.log(`[Step 4] - Register for dynamic Loc event monitoring`);
    runtime2.log(`[Step 4] - Update LC database/registry`);
    runtime2.log(`[Step 4] - Send notifications to buyer and seller`);
    runtime2.log(`[Step 4] - Trigger compliance checks`);
    runtime2.log(`[Step 5] LC issuance processing complete`);
    runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    return lcData;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    runtime2.log(`[ERROR] Failed to process LCIssued event: ${msg}`);
    runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    throw err;
  }
}
...
var initWorkflow = (config) => {
  ...
  return [
    cre.handler(evmClient.logTrigger({
      addresses: [config.evms[0].locManagementAddress],
      topics: [{ values: [LC_ISSUED_EVENT_HASH] }],
